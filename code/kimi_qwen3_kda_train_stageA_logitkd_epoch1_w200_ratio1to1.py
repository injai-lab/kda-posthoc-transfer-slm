...
class TrainConfig:
    ...
    model_revision: str = ""
    init_from_local_dir: str = ""
    output_dir: str = "/root/kda_project/Kimi_Project/qwen3_kda_samecorpus_s1_4k_logitkd_w200_epoch1_ratio1to1"
    ...
    teacher_model_id: str = "Qwen/Qwen3-1.7B-Base"
    teacher_model_revision: str = ""
    kd_logit_weight: float = 0.5
    kd_temperature: float = 2.0
    kd_logit_on_supervised_only: bool = False
    batch_size: int = 1
    grad_accum: int = 4
    num_epochs: int = 2
    max_steps: int = 42521
    ...
    pin_checkpoint_steps: Tuple[int, ...] = (1000, 2000, 5000, 10000, 20000, 30000, 40000, 42521)
...
def patch_rope_scaling(cfg_obj: AutoConfig) -> AutoConfig:
    rope_params = getattr(cfg_obj, "rope_parameters", None)
    rope_scaling = getattr(cfg_obj, "rope_scaling", None)
    ...
    rp = _normalize(rope_params)
    if rp is not None:
        cfg_obj.rope_parameters = rp
        if getattr(cfg_obj, "rope_scaling", None) is None:
            cfg_obj.rope_scaling = dict(rp)
        return cfg_obj

    rs = _normalize(rope_scaling)
    if rs is not None:
        cfg_obj.rope_scaling = rs
        cfg_obj.rope_parameters = dict(rs)
        return cfg_obj

    if hasattr(cfg_obj, "rope_parameters"):
        cfg_obj.rope_parameters = {"rope_type": "default", "type": "default"}
    if getattr(cfg_obj, "rope_scaling", None) is None:
        cfg_obj.rope_scaling = {"rope_type": "default", "type": "default"}
    return cfg_obj
...
def write_kda_reload_manifest(cfg: TrainConfig, final_dir: str) -> str:
    ...
    manifest = {
        ...
        "replace_rule": "alternate_even_layers_kda",
        "kda_config": {
...
if CFG.use_logit_kd:
    ...
    teacher_repo_kwargs = {"trust_remote_code": True, "cache_dir": CFG.cache_dir}
    teacher_rev = (CFG.teacher_model_revision or "").strip() or (CFG.model_revision or "").strip()
    if teacher_rev:
        teacher_repo_kwargs["revision"] = teacher_rev

    teacher_cfg = AutoConfig.from_pretrained(CFG.teacher_model_id, **teacher_repo_kwargs)
    teacher_cfg = patch_rope_scaling(teacher_cfg)
    teacher_cfg.sliding_window = None
...
class KDTrainer(Trainer):
    def __init__(self, *args, teacher_model=None, cfg=None, **kwargs):
        ...
        self.kd_logit_weight = float(getattr(cfg, "kd_logit_weight", 0.0))
        self.kd_temperature = float(getattr(cfg, "kd_temperature", 1.0))
        self.kd_logit_on_supervised_only = bool(getattr(cfg, "kd_logit_on_supervised_only", True))
        self._last_ce_loss = None
        self._last_kd_loss = None
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.teacher_model is not None and self.kd_logit_weight > 0:
            with torch.no_grad():
            ...
            if self.kd_logit_on_supervised_only:
            ...
            token_kl = F.kl_div(s_logp, t_prob, reduction="none").sum(-1)

            mask_f = mask.to(token_kl.dtype)
            kd_loss = (token_kl * mask_f).sum() / mask_f.sum().clamp_min(1.0)
            kd_loss = kd_loss * (T * T)

            loss = ce_loss + self.kd_logit_weight * kd_loss

        self._last_ce_loss = float(ce_loss.detach().cpu())
        self._last_kd_loss = float(kd_loss.detach().cpu()) if kd_loss is not None else None
    ...
    def log(self, logs, *args, **kwargs):
        if self._last_ce_loss is not None:
            logs.setdefault("ce_loss", self._last_ce_loss)
        if self._last_kd_loss is not None:
            logs.setdefault("kd_loss", self._last_kd_loss)
...
class ConsoleLogCallback(TrainerCallback):
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if "ce_loss" in logs:
            msg.append(f"ce={logs['ce_loss']:.4f}")
        if "kd_loss" in logs:
            msg.append(f"kd={logs['kd_loss']:.4f}")
...
if "group_by_length" in args_dict and "group_by_length" not in ta_params:
...
if "do_train" in ta_params:
    args_dict["do_train"] = True
if "do_eval" in ta_params:
...
if teacher_model is not None and CFG.use_logit_kd:
    trainer = KDTrainer(
...
resume_ckpt = locals().get("last_ckpt", None)
