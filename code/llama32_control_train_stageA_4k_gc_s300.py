...
class TrainConfig:
    ...
    output_dir: str = "/root/kda_project/Kimi_Project/llama32_control_nokda_4k_gc_s300"
    final_dir_name: str = "final_stage2_full_kda"
    cache_dir: str = "/root/kda_project/huggingface_cache"
    multi_user_safe: bool = False
    run_name: str = "llama32_control_nokda_4k_gc_s300"
...
def replace_attention_3to1(model: nn.Module, cfg: TrainConfig):
    print("[control] skipping KDA replacement; using original attention stack")
    return 0, 0
...
def set_trainable_params(model: nn.Module, mode: str, train_conv_in_g_only: bool = False) -> int:
    ...
    if trainable == 0:
        print("[control] no KDA params selected; enabling all parameters instead")
    for p in model.parameters():
        p.requires_grad = True
...
for p in student_model.parameters():
    p.requires_grad = True
print("[control] all student parameters set trainable")
