...
class TrainConfig:
    ...
    output_dir: str = "/root/kda_project/Kimi_Project/checkpoints_stage1"
    final_dir_name: str = "final_stage1"
    ...
    grad_accum: int = 8
    num_epochs: int = 2
    max_steps: int = 20  # set <=0 to use full num_epochs
    ...
    skip_if_final_exists: bool = True
    ...
    target_train_windows: int = 20_000
...
def _safe_python_kda_gate(
    ...
    a = a_log.float().view(1, 1, -1, 1)
    decay = (-torch.exp(a)).to(dt.dtype) * dt * float(decay_scale)
    return decay
...
def call_chunk_kda(q, k, v, g, beta, cu_seqlens=None):
...
def call_fused_recurrent_kda(q, k, v, g, beta, cu_seqlens=None):
...
class KimiDeltaAttentionPhi(nn.Module):
    def __init__(
        ...
        self.A_log = nn.Parameter(
            torch.log(torch.empty(num_heads, dtype=torch.float32).uniform_(a_min, a_max))
        )
        ...
        **kwargs,
        ...
        if CFG.force_python_kda_core:
            o, _ = python_delta_rule_kda(
                q=q, k=k, v=v, g=g, beta=beta, initial_state=None, output_final_state=False
            )
        elif mode == "chunk":
            o, _ = call_chunk_kda(q=q, k=k, v=v, g=g, beta=beta, cu_seqlens=cu_seqlens)
        else:
            o, _ = call_fused_recurrent_kda(q=q, k=k, v=v, g=g, beta=beta, cu_seqlens=cu_seqlens)
...
loading_info = {}
if "output_loading_info" in _fp_sig:
    _loaded = AutoModelForCausalLM.from_pretrained(
        CFG.model_id,
        output_loading_info=True,
        **_load_kwargs,
    )
    if isinstance(_loaded, tuple) and len(_loaded) == 2:
        student_model, loading_info = _loaded
    else:
        student_model = _loaded
else:
...
missing_keys = loading_info.get("missing_keys", []) if isinstance(loading_info, dict) else []
unexpected_keys = loading_info.get("unexpected_keys", []) if isinstance(loading_info, dict) else []
critical_missing = [k for k in missing_keys if ("block_sparse_moe" in k or ".mlp." in k)]
critical_unexpected = [k for k in unexpected_keys if ("block_sparse_moe" in k or ".mlp." in k)]
if critical_missing or critical_unexpected:
    raise RuntimeError(
        "Checkpoint/model key mismatch (MoE/MLP)\n"
...
# transformers version compatibility: drop unsupported args
try:
